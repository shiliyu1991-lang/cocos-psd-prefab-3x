...
| 图片 meta | `texture` + 内嵌 sprite-frame（ver 2.3.7） | `image` 类型，`userData.type = 'sprite-frame'`、`imported: false`；内嵌固定子资源 `6c48a`(纹理) / `f9941`(spriteFrame) |
| Sprite 引用 | `_spriteFrame.__uuid__` | `_spriteFrame.__uuid__ = imageUuid@f9941` |

> ⚠️ 图片 meta 的 `userData.type` 必须是 **`sprite-frame`**（不是 `sprite`），且 `imported: false`，否则编辑器不会构建 spriteFrame 子资源，导入后**图片全部丢失**。
