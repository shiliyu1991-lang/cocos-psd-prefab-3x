...
function makeImageMeta(imageUuid, texId, spriteId, name, w, h, slice, hasAlpha) {
    const b = slice || { top: 0, right: 0, bottom: 0, left: 0 };
    return {
        ver: '1.0.27', importer: 'image', imported: true, uuid: imageUuid,
        files: ['.json', '.png'],
        subMetas: {
            [texId]: {
                importer: 'texture', uuid: imageUuid + '@' + texId, displayName: name,
                id: texId, name: 'texture', ver: '1.0.22', imported: true, files: ['.json'], subMetas: {},
                userData: {
            ...
            [spriteId]: {
                importer: 'sprite-frame', uuid: imageUuid + '@' + spriteId, displayName: name,
                id: spriteId, name: 'spriteFrame', ver: '1.0.7', imported: true, files: ['.json'], subMetas: {},
                userData: {
        ...
        userData: { type: 'sprite', hasAlpha: !!hasAlpha, fixAlphaTransparencyArtifacts: false },
    };
}
...
async function convert(opts) {
    ...
    function resolveImage(node) {
        ...
        if (dedup && !node.info.mods.unique && table[hash]) {
            ...
            if (cacheOk && ent.spriteFrameUuid) { report.reused++; return { spriteFrameUuid: ent.spriteFrameUuid, w: png.width, h: png.height }; }
            report.revalidated++; delete table[hash];
        }
        const fname = uniqueFileName(node.name);
        const imageUuid = uuid(), texId = subId(), spriteId = subId();
        const spriteFrameUuid = imageUuid + '@' + spriteId;
        if (!dryRun) {
            fs.writeFileSync(path.join(texDirAbs, fname + '.png'), png.buf);
            fs.writeFileSync(path.join(texDirAbs, fname + '.png.meta'),
                JSON.stringify(makeImageMeta(imageUuid, texId, spriteId, fname, png.width, png.height, sliced, png.hasAlpha), null, 2));
        }
        report.images.push({ name: fname, hash, w: png.width, h: png.height }); report.written++;
        if (dedup && !node.info.mods.unique) table[hash] = { png: path.posix.join(texDirRel, fname + '.png'), imageUuid, spriteFrameUuid, w: png.width, h: png.height };
